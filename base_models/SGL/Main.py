...
class Coach:
	...
	def trainEpoch(self, ep):
		...
		for i, tem in enumerate(trnLoader):
			...
			poss = poss.long().cuda()
			negs = negs.long().cuda()
			usrEmbeds, itmEmbeds, usrEmbeds1, itmEmbeds1, usrEmbeds2, itmEmbeds2 = self.model(self.handler.torchBiAdj_trn, args.keepRate)
			...
			# clLoss = contrastLoss(usrEmbeds1, usrEmbeds2, ancs, args.temp) * args.ssl_reg
	...
	def testEpoch(self):
		...
		with torch.no_grad():
			usrEmbeds, _, _, _, _, _ = self.model(self.handler.torchBiAdj_trn, args.keepRate)
