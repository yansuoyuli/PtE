...
class Coach:
	...
	def trainEpoch(self, ep):
		...
		for i, tem in enumerate(trnLoader):
			...
			poss = poss.long().cuda()
			negs = negs.long().cuda()
			posPreds = self.model.predictPairs(ancs, poss)
			negPreds = self.model.predictPairs(ancs, negs)
			scoreDiff = posPreds - negPreds			
	...
	def testEpoch(self):
		...
		with torch.no_grad():
			for i, batch_data in enumerate(tstLoader):
				...
				numpy_usr = usr.numpy()
				usr = usr.long().cuda()
				preds = self.model.predictAll(usr)
				if args.zero_shot == 0:
					...
					trnMask = torch.sparse.FloatTensor(trnMask, torch.ones(trnMask.shape[1]).cuda(), torch.Size([usr.shape[0], args.item_tst]))
					trnMask = trnMask.to_dense()
					allPreds = preds * (1 - trnMask) - trnMask * 1e8
				else:
					allPreds = preds
