...
class Model(nn.Module):
	...
	def forward(self, adj):
		...
		embeds = torch.concat([self.uEmbeds, F.normalize(item_feats)], axis=0)
		if args.user_aug == 1:
			embeds = torch.concat([self.concat_layer(torch.concat([self.uEmbeds, F.normalize(user_feats)], axis=1)), F.normalize(item_feats)], axis=0)
		...
		return embeds[:args.user], F.normalize(item_feats)
