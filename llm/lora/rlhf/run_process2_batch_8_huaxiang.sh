...
# 定义日志目录
LOG_DIR="logs_process_batch_8/$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"
...
# 第二部分：近端策略优化
# run_step "python rl_training_process.py" "rl_training_process" "近端策略优化" || exit 1
...
run_step "python ../inference_base_mask_process2.py" "inference_base_mask_process" "用户画像生成(含增强)" || exit 1
